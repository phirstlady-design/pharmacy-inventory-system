...
$result = $conn->query("SELECT * FROM codingunit WHERE request_status='released' AND storesection IN ('$store', 'healthStationeryStore')");


 $healthStationeryStore = 'healthStationeryStore';

   // Get total pending count for the specific store section

    $pendingSql = "SELECT COUNT(*) AS total_pending FROM codingunit 
                WHERE request_status = 'released' AND storesection IN (?, ?)";
    ...
    $stmt->bind_param("ss", $store, $healthStationeryStore);
    $stmt->execute();
   ...
   // Get total released value (filtered by store section)
    ...
    $priceSql = "SELECT SUM(currentprice) AS total_price FROM codingunit 
                WHERE request_status = 'released' AND storesection IN (?, ?)";
...
$stmtTotal = $conn->prepare("SELECT COUNT(*) AS total FROM codingunit 
                WHERE request_status = 'released' AND storesection IN (?, ?)");
$stmtTotal->bind_param("ss", $store, $healthStationeryStore);
$stmtTotal->execute();
...
$stmt = $conn->prepare("SELECT * FROM codingunit 
    WHERE request_status = 'released' 
    AND storesection IN (?, ?) 
    LIMIT ?, ?");
$stmt->bind_param("ssii", $store, $healthStationeryStore, $start_from, $results_per_page);
$stmt->execute();
$result = $stmt->get_result();
...
<title>Stationery Release Page</title>
...
<nav class="navbar navbar-expand-lg">
  <div class="container">
    <a class="navbar-brand" href="#"><i class="fas fa-flask me-2"></i>Stationery Release</a>
...
<div class="container main-container">
  ...
  <h1><i class="fas fa-shipping-fast me-2"></i>Stationery Store Release Management</h1>
  <p>Manage and track stationery equipment releases</p>
